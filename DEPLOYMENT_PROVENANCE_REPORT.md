...
|----------------|-----|----------------|-----------|-----|-------|
| (staging D1 schema) | **staging D1** | 2026-07-17 12:44 UTC | migrations 0023–0056 (repo) | — | **UCS schema provisioning** on `cloud-mail-staging` D1 (`acf160ae…`) via `wrangler d1 migrations apply --env staging`. 34 migrations ✅; tables 50→180, 28 `conversation_*`. Additive-only; existing data unchanged. Rollback = drop new objects (pre-snapshot sha256 `18d61e71…`). Runtime OFF. Production untouched. |
| `474ea1fc-f602-4bd8-a947-59cc3c58bdd9` | **staging** | 2026-07-17 UTC | `3b46ecd1dfac4de83a4ca360c3e03b739b8f0c58` | `ucs-outbox-drain-limit-staging-pending` | UCS outbox-drain-limit default-2 deployment. Isolated staging D1/KV/R2, health 200, no cron. Benchmark held: UCS activation/HWM vars absent, so no valid candidate sweep. Rollback ref `d473b56f-079c-4c96-a801-00778c646e5d`. |
