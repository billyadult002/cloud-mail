...
struct MainTabView: View {
        ...
        ZStack(alignment: .top) {
            ...
            TabView(selection: $app.selectedMainTab) {
                ...
                OrganizationCenterView()
                    .tabItem { Label("Organization", systemImage: "building.2.fill") }
                    .tag(5)
            }
    ...
    #if os(iOS)
    @ViewBuilder
    private var currentIOSTab: some View {
        switch app.selectedMainTab {
        case 1:
            AIView()
        case 2:
            WorkCenterView()
    ...
    private var iOSTabBar: some View {
        HStack(alignment: .center, spacing: 0) {
            iOSTabButton(index: 0, title: "Email", icon: "tray.fill")
            iOSTabButton(index: 1, title: "Intelligence", icon: "sparkles")
            iOSTabButton(index: 2, title: "Goals", icon: "target")
            iOSTabButton(index: 4, title: "Trust", icon: "checkmark.shield.fill")
            iOSTabButton(index: 5, title: "Organization", icon: "building.2.fill")
        }
        .padding(.horizontal, 8)
        .frame(maxWidth: .infinity)
        .frame(height: 58)
        .background(VisualSystemV3.ColorToken.surface(for: activeColorScheme).opacity(0.96))
        .overlay(alignment: .top) { Rectangle().fill(VisualSystemV3.ColorToken.hairline(for: activeColorScheme)).frame(height: 0.7) }
    }

    private func iOSTabButton(index: Int, title: String, icon: String) -> some View {
        let selected = app.selectedMainTab == index
        return Button {
            var transaction = Transaction()
            transaction.disablesAnimations = true
            withTransaction(transaction) {
                app.mainTabBarHidden = false
                app.selectedMainTab = index
            }
        } label: {
            VStack(spacing: 3) {
                Image(systemName: icon)
                    .font(.system(size: selected ? 19 : 18, weight: selected ? .semibold : .medium))
            ...
            .foregroundStyle(selected ? VisualSystemV3.ColorToken.accent : VisualSystemV3.ColorToken.tertiaryText(for: activeColorScheme))
            .background {
                if selected {
                    Capsule().fill(VisualSystemV3.ColorToken.accent.opacity(0.12))
                }
            }
            .frame(maxWidth: .infinity)
        ...
        .frame(maxWidth: .infinity)
        // The custom iOS bar fills a 58-point safe-area inset. Make each tab
        // own that whole visible height, rather than leaving an unowned edge
        // around a smaller visual label, so icon and label taps have one
        // consistent target on physical devices and through accessibility.
        .frame(maxHeight: .infinity)
        .contentShape(Rectangle())
        .buttonStyle(.plain)
