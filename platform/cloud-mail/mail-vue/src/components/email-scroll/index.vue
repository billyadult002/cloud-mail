  ...
  <div class="email-container">
    <div class="header-actions">
      ...
      <div class="header-right">
        <div class="mailbox-truth">
          <div class="mailbox-title">{{ currentMailboxTitle }}</div>
          <div class="mailbox-subtitle">{{ currentMailboxSubtitle }}</div>
        </div>
              ...
              <div class="title" :class="accountShow ? 'title-column' : 'title-column'">
                ...
                <div>
                  <div class="source-line">
                    <span :class="'source-badge ' + sourceProviderClass(item)">{{ sourceProviderLabel(item) }}</span>
                    <span class="source-account">{{ sourceAccount(item) }}</span>
                    <span class="source-domain">{{ sourceDomain(item) }}</span>
                  </div>
...
import {htmlToSafeText} from "@/utils/html-sanitizer";
...
onMounted(() => {
  timer = setInterval(() => {
    ...
    const visibleCount = Math.ceil((scrollbarRef.value?.$el?.clientHeight || 0) / itemHeight.value) + 6;
    const startIndex = Math.max(0, Math.floor(scrollTop / itemHeight.value) - 3);
    const endIndex = Math.min(emailList.length, startIndex + visibleCount);
    emailList.slice(startIndex, endIndex).forEach(email => {
...
const itemHeight = computed(() => {
    if (props.type === 'all-email') {
      return isMobile.value ? 148 : 78;
    } else  {
      return isMobile.value ? 106 : 66;
    }
})
...
const currentMailboxTitle = computed(() => {
  if (props.type === 'all-email') return 'All Mail · Merged Inbox';
  const account = accountStore.currentAccount || {};
  const provider = sourceProviderValue(account);
  if (provider === 'gmail') return `Gmail Inbox · ${account.email || 'All Gmail'}`;
  if (account.email) return `CloudMail Inbox · ${account.email}`;
  if (account.allReceive) return 'Merged Inbox';
  return 'Merged Inbox';
})

const currentMailboxSubtitle = computed(() => {
  if (props.type === 'all-email') return 'All providers and mailboxes';
  const account = accountStore.currentAccount || {};
  const provider = sourceProviderValue(account);
  if (provider === 'gmail') return 'Provider Gmail';
  if (account.allReceive) return 'All providers and mailboxes';
  const domain = account.domain || String(account.email || '').split('@')[1] || settingStore.settings.domain || '';
  return domain ? `Domain ${domain}` : 'All mail sources';
})

function sourceProviderValue(item) {
  return String(item.provider || item.accountProvider || '').toLowerCase();
}

function sourceProviderLabel(item) {
  const provider = sourceProviderValue(item);
  if (provider === 'gmail') return 'Gmail';
  return 'CloudMail';
}

function sourceProviderClass(item) {
  return sourceProviderValue(item) === 'gmail' ? 'source-gmail' : 'source-cloudmail';
}

function sourceAccount(item) {
  return item.accountEmail || item.accountEmailFromAccount || item.toEmail || item.email || 'Unknown account';
}

function sourceDomain(item) {
  return item.accountDomain || item.accountDomainFromAccount || String(sourceAccount(item)).split('@')[1] || 'Unknown domain';
}

function htmlToText(email) {
  if (email.content) {
    // Sanitize to plain text without ever inserting live markup into the DOM
    // (no innerHTML sink, no resource loads / handler execution). P0-1 / WF-1.
    let text = htmlToSafeText(email.content);
