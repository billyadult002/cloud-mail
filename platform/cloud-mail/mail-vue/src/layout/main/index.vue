...
import { sanitizeNoticeHtml } from "@/utils/html-sanitizer"
...
function showNotice(data) {
  ...
  // noticeWidth must be numeric — never interpolate raw admin input into CSS.
  const noticeWidth = Number.parseInt(data.noticeWidth, 10) || 420;
  style.textContent = `
  ...
  elNotification = ElNotification({
    ...
    message: `<div style="width: 100%;height: 100%;">${sanitizeNoticeHtml(data.noticeContent)}</div>`,
