...
async function sendEmail() {
  ...
  sendStage.value = form.attachments?.length ? 'Preparing attachments...' : 'Validating message...'
  percent.value = form.attachments?.length ? 8 : 2
  percentMessage = ElMessage({
    message: () => h(sendPercent, {value: percent.value, desc: sendStage.value || t('sending')}),
  ...
  sendStage.value = form.attachments?.length ? 'Uploading attachments...' : t('sending')
  emailSend(form, (e) => {
    percent.value = Math.round((e.loaded * 98) / e.total)
    if (percent.value >= 98) {
      sendStage.value = t('sending')
    }
  }).then(emailList => {
