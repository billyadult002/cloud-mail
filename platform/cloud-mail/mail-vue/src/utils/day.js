...
function clampedLocalTime(time) {
    const now = dayjs();
    const d = dayjs.utc(time).tz(timeZone);
    if (d.diff(now, 'second') > MAX_FUTURE_SKEW_SECONDS) {
        return now;
    }
    return d;
}
