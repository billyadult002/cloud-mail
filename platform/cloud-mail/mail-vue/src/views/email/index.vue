...
async function latest() {
  ...
  while (latestLoopActive) {
    ...
    const foregroundInterval = autoRefresh > 1 ? Math.max(autoRefresh, 60) * 1000 : 60 * 1000;
    await sleep(Math.max(foregroundInterval, latestBackoffMs));

    if (!latestLoopActive || route.name !== 'email' || document.hidden) {
      continue;
    }
      ...
      try {
        ...
        if (accountId === accountStore.currentAccountId && params.timeSort === curTimeSort && allReceive === accountStore.currentAccount.allReceive) {
          if (list.length > 0) {
            ...
            latestBackoffMs = Math.min(latestBackoffMs * 2, 5 * 60 * 1000);
          }
