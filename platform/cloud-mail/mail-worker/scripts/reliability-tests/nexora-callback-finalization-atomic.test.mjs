...
const schema = [
	...
	`CREATE TABLE nexora_provider_outcome_results (id TEXT PRIMARY KEY,tenant_id INTEGER,workspace_id INTEGER,mission_id TEXT,provider TEXT,connection_id TEXT,correlation_id TEXT,committed_token_generation INTEGER,committed_provider_connection_generation INTEGER,outcome_status TEXT)`,
	...
	`CREATE TABLE nexora_onboarding_callback_correlations (id TEXT PRIMARY KEY,tenant_id INTEGER,workspace_id INTEGER,onboarding_mission_id TEXT,status TEXT,consumed_at TEXT,claim_generation INTEGER)`,
...
async function seedValid() {
	...
	await env.db.prepare(`INSERT INTO nexora_onboarding_callback_correlations(id,tenant_id,workspace_id,onboarding_mission_id,status,claim_generation) VALUES('corr-1',?,?, 'mission-1','VERIFIED_PENDING_CONSUMPTION',4)`).bind(scope.tenantId, scope.workspaceId).run();
...
describe('NEXORA callback finalization atomic boundary — production-shaped D1', () => {
	it('commits one callback-only verified result, binds all references, and isolates mission continuation state', async () => {
		...
		expect(await env.db.prepare(`SELECT atomic_callback_result_id,provider_outcome_result_id,token_generation,provider_connection_id,provider_connection_generation,callback_outcome_verified_checkpoint_id FROM nexora_callback_verified_results`).first()).toMatchObject({ atomic_callback_result_id: 'outcome-1', provider_outcome_result_id: 'outcome-1', token_generation: 2, provider_connection_id: 'conn-1', provider_connection_generation: 3, callback_outcome_verified_checkpoint_id: 'nexora-callback-verified:final-1' });
