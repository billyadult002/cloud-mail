...
const dbInit = {
	async init(c) {
		...
		if (String(c.env.CLOUDMAIL_RUNTIME_DDL_ENABLED || '').toLowerCase() !== 'true') {
			return c.json({
				code: 200,
				message: 'runtime_ddl_disabled',
				data: {
					runtimeDdlEnabled: false,
					migrationPath: 'wrangler_d1_migrations',
					note: 'Runtime schema mutation is disabled by default. Apply versioned migrations instead.'
				}
			});
		}
	...
	async v1_6DB(c) {
		...
		await c.env.db.prepare(`UPDATE setting SET notice_content = ? WHERE notice_content = '';`).bind(noticeContent).run();
			try {
				await c.env.db.prepare(`DROP INDEX IF EXISTS idx_account_email`).run();
				await c.env.db.prepare(`DROP INDEX IF EXISTS idx_account_email_nocase`).run();
				await c.env.db.prepare(`DROP INDEX IF EXISTS idx_user_email`).run();
				await c.env.db.prepare(`CREATE UNIQUE INDEX IF NOT EXISTS idx_user_email_nocase ON user (email COLLATE NOCASE)`).run();
				try {
					await c.env.db.prepare(`
