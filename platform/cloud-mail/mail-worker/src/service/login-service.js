...
import cloudMailV2Service from './cloudmail-v2-service';
...
const loginService = {
	...
	async register(c, params, oauth = false) {
		...
		const { salt, hash } = await saltHashUtils.hashPassword(password);
			...
			let routingSetup = { routingCreated: false, action: 'not_attempted' };
			try {
				routingSetup = await cloudMailV2Service.ensureRoutingRule(c, email, userId);
			} catch (error) {
				await c.env.db.prepare(
					`INSERT INTO audit_logs (user_id, actor_role, action, resource_type, outcome, metadata_json)
					 VALUES (?1, 'system', 'cloudflare_routing_rule_setup_failed', 'email_identity', 'failed', ?2)`
				).bind(userId, JSON.stringify({ emailDomain: emailUtils.getDomain(email), code: error.code || 500 })).run();
				routingSetup = { routingCreated: false, action: 'setup_failed' };
			}
		...
		if (registerVerify === settingConst.registerVerify.COUNT && !regVerifyOpen) {
			const row = await verifyRecordService.increaseRegCount(c);
				return {regVerifyOpen: row.count >= regVerifyCount, routingSetup}
			}

			return {regVerifyOpen, routingSetup}
	...
	async login(c, params, noVerifyPwd = false) {
		...
		if (!userRow) {
			const identity = await c.env.db.prepare(
				`SELECT status, user_id FROM email_identities WHERE normalized_email = ?1 LIMIT 1`
			).bind(String(email || '').trim().toLowerCase()).first();
			if (identity?.status === 'active' && identity.user_id) {
				userRow = await userService.selectByIdIncludeDel(c, identity.user_id);
			}
				if (identity && ['pending', 'routing_only', 'verification_required', 'catch_all_eligible'].includes(identity.status)) {
					throw new BizError('该邮箱已存在于你的域名邮箱系统中，请在 NEXORA 中继续激活。');
				}
				if (identity?.status === 'disabled_routing') {
					throw new BizError('该邮箱的收件路由已被其基础设施提供商停用。请返回 NEXORA 查看自动修复或重新授权状态。');
				}
				const discovery = await cloudMailV2Service.discover(c, email);
				if (['routing_only', 'pending', 'catch_all_eligible'].includes(discovery.accountStatus)) {
					throw new BizError(discovery.message);
				}
				if (discovery.accountStatus === 'disabled_routing') {
					throw new BizError(discovery.message);
				}
