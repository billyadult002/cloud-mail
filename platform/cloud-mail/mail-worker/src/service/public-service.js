...
const publicService = {
	...
	async addUser(c, params) {
		...
		for (const emailRow of list) {
			...
			// F1 security repair: all external inputs (email, roleName-derived type,
			// os/browser/device from User-Agent, active_ip) are bound as D1
			// parameters instead of interpolated into the SQL string. ?7 (active_ip)
			// and ?9 (active_time) are reused for their create_ip/create_time twins.
